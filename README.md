...
- **语音与文本记账**：可一次输入多笔消费或收入，逐笔确认后再保存；支持返回上一笔修改。
- **可选 AI 文本解析**：可接入用户自选的兼容 API，将本次文本匹配到本机已有分类。
...
- 目标 Android：API 35
- 文本输入支持本地规则解析；AI 文本解析默认关闭，可在“我的 → AI 文本解析”中主动启用和配置
- AI 可返回多笔待确认草稿，并填写金额、收支、商户、备注、日期和已有分类；不会修改历史账单、分类或分类规则
- 多笔账单支持逐笔前后切换、编辑，并在最后一次确认时原子保存
- 自动支付采集仍取决于设备、系统和第三方应用实际提供的数据；未将第三方支付页解析视为所有机型均已验证
- 账本支持时间、平台、来源、状态组合筛选和月份左右滑动
- 商户分类规则支持新增、查看、编辑、停用和删除
- **1.2.0 修复**：修正多笔账单在第二笔点击“保存全部”时仍使用第一笔回调、无法入账的问题。

## 下载

从 [GitHub Releases](https://github.com/helh1723-lang/moji-android/releases/latest) 下载正式签名 APK。当前正式版为 **v1.2.0**；覆盖安装同签名版本会保留本地账本。首次使用前请阅读应用内的权限说明；从早期 Debug 签名版本迁移时，需要先导出备份。

## 隐私边界

- 账本和规则保存在本地 Room 数据库中。
- AI 文本解析默认关闭。启用后，应用仅向用户自行选择的模型厂商发送本次待解析文本、当前可用分类名称和该厂商 API Key；不会发送历史账单。
- API Key 使用 Android Keystore 加密后只保存于本机，不写入备份、导出或日志。
